...
my ($readsdir, $workdir, $cycle, $K, $P, $NM, $PRIORITY) = @ARGV;

$PRIORITY = 0 unless defined $PRIORITY;

my $GENOME = "$workdir/MERGING/$cycle/ctgs_in.fa";
my $outdir = "$workdir/MERGING/$cycle/map";
...
open  QSUB, ">$workdir/MERGING/$cycle/bwt.sh";
...
open SH, ">$workdir/MERGING/$cycle/dummy_bwt.sh";
print SH "\#!/bin/sh\n\#\$ -S /bin/sh\n";
close SH;

print QSUB "qsub -P wga -o $workdir/MERGING/$cycle -e $workdir/MERGING/$cycle  -sync y -hold_jid ",join(",",@jobs_to_wait)," $workdir/MERGING/$cycle/dummy_bwt.sh";
close QSUB;

system("sh $workdir/MERGING/$cycle/bwt.sh");

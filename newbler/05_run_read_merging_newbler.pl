...
my $MERGING_MAX_READS = $CONF{'MERGING_MAX_READS'};
my $MERGING_MIN_READS = $CONF{'MERGING_MIN_READS'};
my $MERGING_WINDOW = $CONF{'MERGING_WINDOW'};
...
foreach my $p ( @to_run ) {
    ...
    $p=~s/.*\///;
    print SH "$workdir/normalize_coverage_and_merge_reads_multi_newbler.pl $p LIBNAMES $MERGING_MAX_READS $MERGING_WINDOW $MERGING_MIN_READS\n";

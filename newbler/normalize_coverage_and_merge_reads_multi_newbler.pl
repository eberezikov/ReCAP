...
my ($ctgs,$libnames,$max_reads_per_position,$window_for_merging,$reads_per_pos_skip_merging) = @ARGV;

#my $cap3params = "-i 30 -j 31 -o 18 -s 300 -v 1 -k 0";

$max_reads_per_position = 100 unless defined $max_reads_per_position;
$window_for_merging = 1 unless defined $window_for_merging;
$reads_per_pos_skip_merging = 30 unless defined $reads_per_pos_skip_merging;
...
while (my $ctgid = <BLOCK>) {
  ...
  my ($unqid) = $ctgid=~/^\S+?-(\S+)/;
  $current++;
#  unless ($ctgid eq 'contig1-1089628') {
#      while (my $l = <BLOCK>) {
#        last if $l=~/^\n/;
#      }
#      next;
#  }
  ...
  for my $reduce (['posF',\%POS1,'2'], ['posR',\%POS2,'1']) {
       ...
       foreach my $p (@pos) {
          ...
          my $nreads_to_merge = scalar keys %THISREADS;
#          print "$current/$ctgs_to_process: $ctgid ($ctglen): ",($p-1)*$window_for_merging,"-",$p*$window_for_merging," $dir $pn/$totpos,$nreads_to_merge reads\n";
          if ($nreads_to_merge <= $reads_per_pos_skip_merging) { #skip merging, keep all original reads here
             foreach my $id (keys %THISREADS) {
                  $READS_NOT_MERGED{$id}=$THISREADS{$id}{'1'}.$THISREADS{$id}{'2'}.$THISQUAL{$id}{'1'}.$THISQUAL{$id}{'2'}.$THISCON{$id};
             }
             print "$current/$ctgs_to_process: $ctgid ($ctglen): ",($p-1)*$window_for_merging,"-",$p*$window_for_merging," $dir $pn/$totpos,$nreads_to_merge reads: merging skipped\n";
          }
          else {
            ...
            if (@reads_used) {
                ...
                print "$current/$ctgs_to_process: $ctgid ($ctglen): ",($p-1)*$window_for_merging,"-",$p*$window_for_merging," $dir $pn/$totpos,$nreads_to_merge reads: merged, ",scalar @reads_used," reads used\n";
            }
            else {
                foreach my $id (keys %THISREADS) {
                  $READS_NOT_MERGED{$id}=$THISREADS{$id}{'1'}.$THISREADS{$id}{'2'}.$THISQUAL{$id}{'1'}.$THISQUAL{$id}{'2'}.$THISCON{$id};
                }
                print "$current/$ctgs_to_process: $ctgid ($ctglen): ",($p-1)*$window_for_merging,"-",$p*$window_for_merging," $dir $pn/$totpos,$nreads_to_merge reads: not merged\n";
            }
           `rm -r *`; #cleanup after each pos            
          }
       }
       chdir ".."; 
  }
...
sub assemble_pairs {
    ...
    my ($s,$q,$con)=("","","");
    my %REPORTED;
    `/opt/454/bin/runAssembly -cdna -urt -nobig -ace -o $p.$d.nb $p.$d.fa >$p.$d.log 2>&1`;
    return($s,$q,$con,$pairn,keys %REPORTED) unless (-e "$p.$d.nb/454Isotigs.ace");
    open(ACE,"$p.$d.nb/454Isotigs.ace") || die "!!! no $p.$d.nb/454Isotigs.ace\n";
    local $/ = undef;
    my @ace = split(/\n\n+/,<ACE>);
    close ACE;
    local $/ ="\n";
    ...
    foreach my $ctg ($highest_ctg[0]->[0]) { #only highest coverage ctg is considered
      ...
      while ($CTGINFO{$ctg}{'AF'}=~/AF (\S+?)\.(\S)\S* (\S) (\d+)/g) {
      ...
      while (1) {
        `/opt/454/bin/runAssembly -cdna -urt -nobig -ace -o $p.$ctg.$j.1.nb $p.$ctg.$j.1 >$p.$ctg.$j.1.log 2>&1`;
        `/opt/454/bin/runAssembly -cdna -urt -nobig -ace -o $p.$ctg.$j.2.nb $p.$ctg.$j.2 >$p.$ctg.$j.2.log 2>&1`;
         my ($ctg1,$qual1,$RD1,$remove1) = read_ace_Contig1("$p.$ctg.$j.1.nb/454Isotigs.ace","r","U");
         my ($ctg2,$qual2,$RD2,$remove2) = read_ace_Contig1("$p.$ctg.$j.2.nb/454Isotigs.ace","f","U");
         my %COMMONREADS;
         my $missed=0;
         my %FLANKS;
...
sub read_ace_Contig1 {
    ...
    my $ctgseq="";
    my $ctgqual="";
    return ($ctgseq,$ctgqual,\%RD,$remove) unless (-e $ace);
    ...
    if (defined $THISCTG{'isotig00001'}) {
      $ctgseq=$THISCTG{'isotig00001'}{'CO'};
      $ctgseq=~s/.*\n//; $ctgseq=~s/\n//g; $ctgseq=~s/\*//g;
      $ctgqual=$THISCTG{'isotig00001'}{'BQ'};
      $ctgqual=~s/.*\n//; $ctgqual=~s/\s+/ /g;
      while ($THISCTG{'isotig00001'}{'AF'}=~/AF (\S+?)\.(\S)\S* (\S) (\d+)/g) {
         my ($id,$direction,$orientation,$start) = ($1,$2,$3,$4);
...
sub count_qual {
  ...
  return 100; #skip quality selection
